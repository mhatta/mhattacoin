...
const char * const BITCOIN_CONF_FILENAME = "mhattacoin.conf";
const char * const BITCOIN_PID_FILENAME = "mhattacoin.pid";
...
#else
    const char* pszModule = "mhattacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MHattaCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MHattaCoin";
#else
    // Unix
    return pathRet / ".mhattacoin";
#endif
#endif
}
